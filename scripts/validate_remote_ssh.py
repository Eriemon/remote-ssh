...
def run_tool(args: list[str], expected: int | set[int] = 0, input_text: str | None = None) -> subprocess.CompletedProcess[str]:
...
def fake_scan_output() -> str:
    return "\n".join(
        [
            ...
            "software:cuda:installed:/usr/local/cuda/bin/nvcc:Cuda compilation tools, release 12.2:",
...
def software_scan_tests(settings: dict[str, Any], tmp_dir: Path) -> None:
    ...
    assert_contains(software_result.stdout, "path: /usr/local/cuda/bin/nvcc", "cached cuda output")
...
def request_and_path_tests(settings: dict[str, Any], settings_path: Path, server_list: Path, ref_config: dict, tmp_dir: Path) -> None:
    ...
    upload_request = latest_request(requests_root)
    ...
    delete_request = latest_request(requests_root)
...
def public_artifact_boundary_audit() -> None:
    gitignore = SKILL_DIR / ".gitignore"
    if gitignore.exists():
        raise ValidationError("public remote-ssh repository must not include the bundled .gitignore")

    forbidden_paths = [
        SKILL_DIR / "config" / "server_list.local.json",
        SKILL_DIR / "requests",
        SKILL_DIR / "downloads",
        SKILL_DIR / "tmp",
        SKILL_DIR / "logs",
    ]
    present = [path.relative_to(SKILL_DIR).as_posix() for path in forbidden_paths if path.exists()]
    present.extend(path.relative_to(SKILL_DIR).as_posix() for path in (SKILL_DIR / "config").glob("*.bak*"))
    present.extend(path.relative_to(SKILL_DIR).as_posix() for path in SKILL_DIR.rglob("*.log"))
    if present:
        raise ValidationError(f"public artifact contains local operational files: {', '.join(sorted(present))}")
...
def main() -> int:
    ...
    try:
        ...
        public_artifact_boundary_audit()
